...
from utils.data_loader_predict import get_loader_predict
...
def test(config):
    ...
    for filename in os.listdir(config.bf_input_path):
        ...
        # bf_gt = np.expand_dims(bf_gt, -1)
        # bf_gt = np.expand_dims(bf_gt, 0)

        predict_bf = bf_network.predict(bf_input)
        predict_vn = vn_network.predict(predict_bf)
        # predict_vn = vn_network.predict(bf_gt)
        ...
        #                   os.path.join(config.result_path, config.test_dir + '_chimap_' + config.name_vn + '_' + name))
...
if __name__ == '__main__':
    ...
    # experiment info
    parser.add_argument('--bf_input_path', type=str, default='./data/real_totalfield_rerange/')
    parser.add_argument('--bf_gt_path', type=str, default='./data/real_localfield_rerange/')
    ...
    parser.add_argument('--model_num_bf', type=str, default='100')
    parser.add_argument('--model_num_vn', type=str, default='100')

    parser.add_argument('--result_path', type=str, default='./predict_result/')
    parser.add_argument('--test_dir', type=str, default='version1_rerange')
