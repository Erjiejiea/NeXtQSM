...
def main(config):
    ...
    print('Loaded {} samples for training.'.format(tf.data.experimental.cardinality(train_dataset).numpy() * config.BATCH_SIZE))
    print('Loaded {} samples for validation.'.format(tf.data.experimental.cardinality(val_dataset).numpy() * config.BATCH_SIZE))

    test_dataset = gen_dataset_test(os.path.join(config.realdata_path, 'totalfield'),
                                    os.path.join(config.realdata_path, 'localfield'))
    print('Loded {} real brain.'.format(tf.data.experimental.cardinality(test_dataset).numpy()))
    ...
    with tf.device('/GPU:0'):
        ...
        bf_history = bf_network.fit(train_dataset,
                                    ...
                                    shuffle=True,
                                    validation_data=val_dataset)  # pass callback to training for saving the model
...
if __name__ == '__main__':
    ...
    parser.add_argument('--localfield_path', type=str, default='/DATA_Temp/cj/QSM/NeXtQSM/train_localfield/')
